...
<article class="am-article am-margin-top-sm">
    <div class="am-article-bd task-edit-display">
        <?php if (empty($task_content)): ?>
            <p>The founder guy was lazy and didn't leave any details. Maybe it's a quick task.</p>
        <?php else: ?>
            ...
            <?php foreach ($supplement as $supplementContent): ?>
                ...
                <div class="task-date am-text-center">
                    <span class="task-date-align ">
                        <?= $label->findContent('user', 'user_id', $supplementContent['task_supplement_user_id'])['user_name']; ?> Supplement <?= date('Y/m/d H:i', $supplementContent['task_supplement_createtime']); ?>
                        <?php if ($this->session()->get('team')['user_id'] == $supplementContent['task_supplement_user_id'] && !in_array($task_status, ['3', '10']) ): ?>
                            <a href="<?= $label->url('Team-Task_supplement-action', ['id' => $supplementContent['task_supplement_id'], 'task_id' => $task_id, 'method' => 'DELETE', 'back_url' => base64_encode($_SERVER['REQUEST_URI'])]); ?>" class="ajax-click ajax-dialog"><i class="am-icon-recycle"></i>Remove</a>
                        <?php endif; ?>
                    </span>
                </div>
