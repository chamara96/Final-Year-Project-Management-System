        ...
        <ul class="am-dropdown-content">
            <?php foreach ($statusMark as $status): ?>
                ...
                <li class="<?= $disabled ?>" status="<?= $status['task_status_type']; ?>">
                    <a class="ajax-click ajax-dialog <?= $disabled ?>" msg="Are you sure you want to change the status of the task?" href="<?= $this->url('Team-Task-status', ['task_id' => $task['task_id'], 'status' => $status['task_status_type'], 'method' => 'PUT', 'back_url' => base64_encode($_SERVER['REQUEST_URI'])]); ?>"><i class="<?= $status['task_status_icon']; ?>"></i> <?= $status['task_status_name']; ?>
                    </a>
                </li>
