...
<?php if (empty($list)): ?>
    <div class="am-margin-top-sm"><i class="am-icon-coffee"></i> No matching missions found~</div>
<?php else: ?>
    <?php foreach ($list as $key => $value): ?>
        ...
        <div class="task-list">
            ...
            <div class="am-margin-bottom-xs">#<?= $value['task_id'] ?>
                ...
                <?php endif; ?>
                <?= $value['task_multiplayer'] == '1' ? 'Waiting for many people' : '' ?>
                ...
                <span
                    ...
                    title="Planned completion time:<?= date('m-d H:i', $value['task_end_time']) ?>"><i
                        class="am-icon-calendar"></i> <?= date('m.d', $value['task_end_time']) ?></span>
                <!--任务计划完成事件-->

                <?php if($label->checkAuth('TeamDELETETaskaction') === true): ?>
                <a class="am-text-danger ajax-click ajax-dialog"  msg="confirm to delete? Will not recover!" href="<?= $label->url(GROUP . '-' . MODULE . '-action', array('id' => $value["task_id"], 'method' => 'DELETE', 'back_url' => base64_encode($_SERVER['REQUEST_URI']))); ?>"><span class="am-icon-trash-o"></span></a>
                <?php endif; ?>
            </div>
