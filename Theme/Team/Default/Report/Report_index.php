    ...
    <div class="am-panel-bd">
        <form action="<?= $label->url('Team-Report-action') ?>" class="ajax-submit" method="POST">
            ...
            <button type="submit" class="am-btn am-radius am-btn-warning am-btn-xs">
                <i class="am-icon-hand-pointer-o"></i> Submit report
            </button>
        </form>
...
<?php if (!empty($list)): ?>
    <ul class="am-list am-list-border am-list-striped">
        <?php foreach ($list as $key => $value): ?>
            <li><a href="<?= $label->url('Team-Report-view', ['id' => $value['report_id'], 'back_url' => base64_encode($_SERVER['REQUEST_URI'])]); ?>"><?= date('Y-m-d', $value['report_date']); ?>Report</a></li>
        <?php endforeach; ?>
    </ul>
